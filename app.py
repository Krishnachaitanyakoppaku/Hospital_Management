...
app.config['SESSION_COOKIE_SECURE'] = False  # Allow non-HTTPS connections
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = None  # Allow cross-site requests

# PostgreSQL Database Configuration
# Use environment variable for database URL (Supabase format)
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'postgresql://postgres:password@localhost:5432/hospital_db')
...
# Make sure database session is initialized with each request
@app.before_request
def before_request():
...
def recreate_db():
    with app.app_context():
        # Drop tables in the correct order (CASCADE for PostgreSQL)
        db.session.execute(text('DROP TABLE IF EXISTS appointment CASCADE'))
        db.session.execute(text('DROP TABLE IF EXISTS date_specific_slot CASCADE'))
        db.session.execute(text('DROP TABLE IF EXISTS doctor_slot CASCADE'))
        db.session.execute(text('DROP TABLE IF EXISTS admitted_patient CASCADE'))
        db.session.execute(text('DROP TABLE IF EXISTS doctor CASCADE'))
        db.session.execute(text('DROP TABLE IF EXISTS patient CASCADE'))
        db.session.commit()
...
def index():
    ...
    # Always show the index page with login/register options
...
def admin_dashboard():
    ...
    admitted_patients = AdmittedPatient.query.filter_by(status='admitted').all()
    admitted_patient_ids = set(patient.patient_id for patient in admitted_patients if patient.patient_id)
    
    # Filter patients to only those with completed or cancelled appointments and not admitted
    patients_with_appointments = set(appointment.patient_id for appointment in appointments)
    filtered_patients = [p for p in patients if p.id in patients_with_appointments and p.id not in admitted_patient_ids]
...
def delete_doctor(doctor_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': f'Error deleting doctor: {str(e)}'}), 500
...
def edit_doctor(doctor_id):
    ...
    if request.method == 'POST':
        try:
            ...
            doctor.years_of_experience = int(request.form.get('years_of_experience'))
            ...
            db.session.rollback()
            flash(f'Error updating doctor information: {str(e)}', 'danger')
...
def admin_book_appointment():
    ...
    if request.method == 'POST':
        ...
        if not patient:
            # Create a temporary patient using their actual name
            # Convert patient name to lowercase and replace spaces with underscores for username
            base_username = patient_name.lower().replace(' ', '_')
            username = base_username
            
            # If username exists, append a number
            counter = 1
            while Patient.query.filter_by(username=username).first():
...
def view_all_patients():
    ...
    # Base query for all patients (excluding admin users)
    patients_query = Patient.query.filter_by(is_admin=False)
    
    # Apply filters if provided
    if patient_name:
        patients_query = patients_query.filter(Patient.username.ilike(f'%{patient_name}%'))
    ...
    # Filter patients to only those with completed or cancelled appointments
    patients_with_appointments = set(appointment.patient_id for appointment in all_appointments)
    
    # Get all admitted patients
    admitted_patients = AdmittedPatient.query.filter_by(status='admitted').all()
    admitted_patient_ids = set(patient.patient_id for patient in admitted_patients if patient.patient_id)
    
    # Filter patients to exclude those who are currently admitted
    patients = [p for p in patients if p.id in patients_with_appointments and p.id not in admitted_patient_ids]
    
    # Organize appointments by patient
    for patient in patients:
        patient.completed_appointments = []
        patient.pending_appointments = []
        patient.cancelled_appointments = []
        patient.display_name = None  # Initialize display name
        
        for appointment in all_appointments:
            if appointment.patient_id == patient.id:
                # Set display name from first appointment with a patient_name
                if not patient.display_name and appointment.patient_name:
                    patient.display_name = appointment.patient_name
                
                if appointment.status == 'completed':
                    patient.completed_appointments.append(appointment)
                elif appointment.status == 'cancelled':
                    patient.cancelled_appointments.append(appointment)
...
def edit_patient(patient_id):
    ...
    if request.method == 'POST':
        ...
        if not username or not email:
            ...
            if appointments:
                current_display_name = appointments[0].patient_name
            return render_template('edit_patient.html', patient=patient, display_name=current_display_name)
        
        # Check if username already exists for a DIFFERENT patient (if changed)
        existing_user = Patient.query.filter_by(username=username).first()
        if username != patient.username and existing_user and existing_user.id != patient_id:
        ...
        # Check if email already exists for a DIFFERENT patient (if changed)
        existing_email = Patient.query.filter_by(email=email).first()
        if email != patient.email and existing_email and existing_email.id != patient_id:
        ...
        # Update appointment display name data if provided
        if display_name:
            appointments = Appointment.query.filter_by(patient_id=patient_id).all()
            for appointment in appointments:
                appointment.patient_name = display_name
    ...
    # Get display name from any appointment
    appointments = Appointment.query.filter_by(patient_id=patient_id).all()
    display_name = None
    if appointments:
...
if __name__ == '__main__':
    # This is only for development - use wsgi.py for production
    try:
        # Initialize the database
        init_db()
        ...
        print("⚠️  Running in development mode. For production, use: gunicorn wsgi:app")
